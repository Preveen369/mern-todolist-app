...
const TodoList = ({ todos, fetchTodos }) => {
  const handleToggle = async (id, completed) => {
    try {
      await axios.put(`${process.env.REACT_APP_API_BASE_URL}/api/todos/${id}`, { completed: !completed });
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_BASE_URL}/api/todos/${id}`); // Use proxy
      fetchTodos();
    } catch (error) {
